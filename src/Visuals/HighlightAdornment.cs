		...
		private void CreateVisuals(ITextViewLine line)
		{
			...
			if (docFactory.TryGetTextDocument(line.Snapshot.TextBuffer, out ITextDocument document))
			{
				...
				foreach (var highlight in highlights)
				{
					...
					{
						//result.Add(new ClassificationSpan(highlightSpan, this.classificationType));

						//yield return new TagSpan<TodoTag>(highlightSpan, new TodoTag());

						// TODO: need to handle highlights that don't cover the whole line.
						SnapshotSpan span = new SnapshotSpan(this.view.TextSnapshot, lineSpan);
						...
						if (geometry != null)
						{
							...
							switch (highlight.Color)
							{
								case HighlightColor.DarkTurquoise:
									brush = this.darkTurquoiseBrush;
									break;
								case HighlightColor.Fuchsia:
									brush = this.fuchsiaBrush;
									break;
								case HighlightColor.Gold:
									brush = this.goldBrush;
									break;
								case HighlightColor.Lime:
								default:
									brush = this.limeBrush;
									break;
							}
